...
Steam_rating <- Games_Tidy %>%
  mutate(Steam_rating = as.numeric(sub("%", "", `Steam Rating`)) * 100)

genre_rating <- list()

for (genre in my_rating_list) {
  # Get revenue for the current genre
  genre_rating[[genre]] <- mean(DS_GetAllOf(Games_Tidy, "Genres", genre)$'Steam Rating' * 100)
}
...
ggplot(rating_data, aes(x = reorder(Genre, Rating), y = Rating, fill = Genre)) +
  geom_bar(stat = "identity", alpha = 0.9) +
  geom_text(aes(label = format(Rating, scientific = FALSE)), vjust = -0.5, size = 2.1, color = "black") + 
  labs(title = "Avg Steam Rating per Genre", x = "Genre", y = "Percentage Rating") +
  scale_fill_manual(values = genre_colors) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(labels = function(x) format(x, scientific = FALSE)) 
```
